...
from cerulean_sonar_ros.cfg import S500Config
...
class S500Driver:
	def __init__(self):
		...
		self.ping_delay_ms = 1000 / float(rospy.get_param('~ping_rate_hz', 1.0))
		self.frame_id = rospy.get_param('~frame_id', 'sonar_link')
		self.field_of_view = rospy.get_param('~field_of_view', 0.0872665) #5 deg
		self.use_chirp = rospy.get_param('~use_chirp', False)
		
		# Create publisher
		self.range_pub = rospy.Publisher('/echosounder/range', Range, queue_size=10)
		...
		self.range_msg.max_range = 100.0  # 100m
		self.range_msg.header.frame_id = self.frame_id

		self.enabled_sub = rospy.Subscriber("/echosounder/enabled", Bool, self.enabled_callback)
		self.enabled_pub = rospy.Publisher("/echosounder/enabled", Bool, queue_size=1, latch=True)
		self.enabled_pub.publish(self.enabled)

		self.reconf_server = Server(S500Config, self.reconf_callback)
	...
	def reconf_callback(self, config, level):
		rospy.loginfo(f"Reconfigure request: ping_rate_hz={config.ping_rate_hz}, use_chirp={config.use_chirp}")

		# Update internal parameters
		self.ping_rate_hz = config.ping_rate_hz
		self.ping_delay_ms = 1000.0 / float(self.ping_rate_hz)
		self.use_chirp = config.use_chirp

		# Restart sonar with new settings
		self.stop_pinging()
		time.sleep(1.0)
		self.start_pinging()
	...
	def start_pinging(self):
		...
		chirp = 1 if self.use_chirp else 0
		...
		params = struct.pack('<IIhhHHHBB', 
			...
			-1,     # gain_index (auto)
			round(self.ping_delay_ms),    # msec_per_ping (10Hz)
